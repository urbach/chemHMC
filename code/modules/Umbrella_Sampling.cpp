...
Umbrella_Sampling::Umbrella_Sampling(const YAML::Node& config)
{
    ...
    if (config["density"]) {
        ...
        density_spring_constant = config["density"]["spring_constant"].as<double>();
        density_center = config["density"]["center"].as<double>();
    ...
    if (config["Q6"]) {
        ...
        Q6_enabled = true;
        Q6_spring_constant = config["Q6"]["spring_constant"].as<double>();
...
void Umbrella_Sampling::init(const particles_instance& particles)
{
    ...
    trial_Q6 = Q6;
    bias_energy = compute_bias_energy(density, Q6, particles.T);
...
void Umbrella_Sampling::evaluate_trial(const particles_instance& particles)
{
    ...
    trial_bias_energy = compute_bias_energy(trial_density, trial_Q6, particles.T);
}
...
void Umbrella_Sampling::write_output(int step)
{
    ...
    output << step << " "
           << density << " "
           << density_spring_constant << " "
           << density_center << " "
           << Q6 << " "
           << Q6_spring_constant << " "
           << Q6_center << " "
           << bias_energy << std::endl;
}

double Umbrella_Sampling::compute_bias_energy(double density_value, double Q6_value, double temperature) const
{
    double energy = 0.0;
    if (density_enabled) {
        double difference = density_value - density_center;
        energy += 0.5 * kB * temperature * density_spring_constant * difference * difference;
    }
    if (Q6_enabled) {
        double difference = Q6_value - Q6_center;
        energy += 0.5 * kB * temperature * Q6_spring_constant * difference * difference;
    }
    return energy;
}
