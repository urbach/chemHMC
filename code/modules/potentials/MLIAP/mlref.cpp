...
void MLIAP::init(const particles_instance& particles) {
    ...
    if (module_.hasattr("compute_forces")) {
        try { module_.setattr("compute_forces", true); }
        catch (const std::exception& e) {
            std::cerr << "Failed to set compute_forces: " << e.what() << "\n";
        }
    }
}
...
double MLIAP::potential(const particles_instance& particles) {
    try
    {
        ...
        // Edge index shape: (2, N_edge). For simplicity create fully connected (excluding self)
        // We'll define undirected edges explicitly (i->j and j->i)
        std::vector<int64_t> row_src;
        std::vector<int64_t> row_dst;
        for (int64_t i = 0; i < N_at; ++i) {
            for (int64_t j = 0; j < N_at; ++j) {
                if (i == j) continue;
                row_src.push_back(i);
                row_dst.push_back(j);
            }
        }
        int64_t N_edge = static_cast<int64_t>(row_src.size());
        torch::Tensor edge_index = torch::stack({
            torch::from_blob(row_src.data(), {N_edge}, torch::TensorOptions().dtype(torch::kInt64)).clone(),
            torch::from_blob(row_dst.data(), {N_edge}, torch::TensorOptions().dtype(torch::kInt64)).clone()
        }, 0).to(device_);

        std::cout << "Edge count: " << N_edge << std::endl;
        std::cout << "Edge index:\n" << edge_index.cpu() << std::endl;
...
void MLIAP::force(const particles_instance& particles, type_f& f) {
    try
    {
        ...
        std::cout << "Forces shape: " << forces.sizes() << "\n";
        std::cout << forces << "\n";
