    ...
    { // checking whether the file is open
        ...
        { // read data from file object and put it into string.
            ...
            { // if not empty line
                if (x.size() != 2)
                {
                    printf("error infile scan for %s\n expected:  param = value  \n found: %s \n", name.c_str(), tp.c_str());
                    exit(-9);
                }
...
void read_par_string(std::fstream &newfile, std::string name, std::string &s, bool required = true)
{
    ...
    if (match == 0)
    {
        if (required)
        {
            std::cout << "could not find line: " << name.c_str() << " =" << std::endl;
            exit(-10);
        }
        else
...
params_class::params_class(int argc, char **argv)
{
    int opt = -1;
    ...
    else
    {
        ...
        std::cout << " Lx: "<< doc["geometry"]["Lx"].as<double>()<< std::endl;
        std::cout << " Ly: "<< doc["geometry"]["Ly"].as<double>()<< std::endl;
        std::cout << " Lz: "<< doc["geometry"]["Lz"].as<double>()<< std::endl;
        L[0]=doc["geometry"]["Lx"].as<double>();
        L[1]=doc["geometry"]["Ly"].as<double>();
        L[2]=doc["geometry"]["Lz"].as<double>();

        Nathoms=doc["athoms"]["Nathoms"].as<int>();
    }
    // std::fstream newfile;
